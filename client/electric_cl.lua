if Config.ElectricVehicleCharging then
    ...
    function SetElectricNozzle(state)
        ...
        elseif state == "pickup" then    
            ...
            if Config.FuelDebug then lib.print.debug("State is not valid, it must be pickup or putback.") end
        end
    end 
    ...
    RegisterNetEvent('cdn-fuel:client:electric:FinalMenu', function(purchasetype)
        ...
        if Config.FuelDebug then lib.print.debug("Attempting to open Input with the total: $"..wholetankcostwithtax.." at $"..FuelPrice.." / L".." Maximum Fuel Amount: "..maxfuel) end
        ...
        if Electricity then
            if not ElectricityAmount then if Config.FuelDebug then lib.print.debug("ElectricityAmount is invalid!") end return end
            if not HoldingElectricNozzle then exports.qbx_core:Notify(locale("electric_no_nozzle"), 'error', 7500) return end
    ...
    RegisterNetEvent('cdn-fuel:client:electric:SendMenuToServer', function()
        ...
        if not AwaitingElectricCheck then if Config.FuelDebug then lib.print.debug("Attempting to go to Charging Menu") end end
        if not AwaitingElectricCheck and FoundElectricVehicle then
            ...
            if not IsHoldingElectricNozzle() then exports.qbx_core:Notify(locale("electric_no_nozzle"), 'error', 7500)  return end
            if CurFuel < 95 then
            ...
            if Config.FuelDebug then lib.print.debug("Checking") end
            if AwaitingElectricCheck then
                if Config.ElectricVehicles[vehiclename] and Config.ElectricVehicles[vehiclename].isElectric then
                    ...
                    if Config.FuelDebug then lib.print.debug("^2"..current.. "^5 has been found. It ^2matches ^5the Player's Vehicle: ^2"..vehiclename..". ^5This means charging will be allowed.") end
                    Wait(50)
                    ...
                    AwaitingElectricCheck = false
                    Wait(50)
                    if Config.FuelDebug then lib.print.debug("^2An electric vehicle^5 has NOT been found. ^5This means charging will not be allowed.") end
                    goto ChargingMenu -- Attempt to go to the charging menu, now that we have not found that there was an electric vehicle.
                end
            else
                exports.qbx_core:Notify(locale("electric_vehicle_not_electric"), 'error', 7500)
            end
        end
    end)

    RegisterNetEvent('cdn-fuel:client:electric:ChargeVehicle', function(data)
        if Config.FuelDebug then lib.print.debug("Charging Vehicle") end
    ...
    RegisterNetEvent('cdn-fuel:client:grabelectricnozzle', function()
        ...
        CreateThread(function()
            while HoldingElectricNozzle do
                ...
                if not TargetCreated then return end
                TargetCreated = true
                if dist > 7.5 then
                    ...
                    if Config.PumpHose == true then
                        if Config.FuelDebug then lib.print.debug("Removing ELECTRIC Rope.") end
                        RopeUnloadTextures()
                        DeleteRope(Rope)
                    end
                end
                ...
                if Config.RenewedPhonePayment then
                    if not Cancelledrefuel and not RefuelCancelled then
                        if RefuelPossibleAmount then
                            ...
                            if Config.FuelDebug then lib.print.debug("Attempting to charge vehicle.") end
                            TriggerEvent('cdn-fuel:client:electric:ChargeVehicle', purchasetype, fuelamounttotal)
                        else
                            exports.qbx_core:Notify(locale("electric_more_than_zero"), 'error', 7500)
                        end
                    end
                end
            end
        ...
        RegisterNetEvent('cdn-fuel:client:electric:phone:PayForFuel', function(amount)
            ...
            if success then
                if QBX.PlayerData.money['bank'] <= (GlobalTax(amount) + amount) then
                    exports.qbx_core:Notify(locale("not_enough_money_in_bank"), "error")
                else
    ...
    if Config.ElectricChargerModel then
        CreateThread(function()
            RequestModel(Config.ElectricChargerModelName)
            while not HasModelLoaded(Config.ElectricChargerModelName) do
                Wait(50)
            end

            if Config.FuelDebug then
                lib.print.debug("Electric Charger Model Loaded!")
            end

            for i = 1, #Config.GasStations do
                if Config.GasStations[i].electricchargercoords ~= nil then
                    if Config.FuelDebug then lib.print.debug(i) end
                    local heading = Config.GasStations[i].electricchargercoords[4] - 180
                    Config.GasStations[i].electriccharger = CreateObject(Config.ElectricChargerModelName, Config.GasStations[i].electricchargercoords.x, Config.GasStations[i].electricchargercoords.y, Config.GasStations[i].electricchargercoords.z, false, true, true)
                    if Config.FuelDebug then lib.print.debug("Created Electric Charger @ Location #"..i) end
    ...
    exports.ox_target:addModel(Config.ElectricChargerModelName, modelOptions)
end