...
function IsPlayerNearVehicle()
	if Config.FuelDebug then
		lib.print.debug("Checking if player is near a vehicle!")
	end
...
function IsVehicleBlacklisted(veh)
	if Config.FuelDebug then lib.print.debug("IsVehicleBlacklisted("..tostring(veh)..")") end
	if veh and veh ~= 0 then
		veh = string.lower(GetDisplayNameFromVehicleModel(GetEntityModel(veh)))
		if Config.FuelDebug then lib.print.debug("Vehicle: "..veh) end
		...
		if not Config.ElectricVehicleCharging then
			if Config.ElectricVehicles[veh] and Config.ElectricVehicles[veh].isElectric then
				if Config.FuelDebug then lib.print.debug("Vehicle: "..veh.." is in the Blacklist.") end
				return true
			end
		end
		...
		-- Default False
		if Config.FuelDebug then lib.print.debug("Vehicle is not blacklisted.") end
		return false
	else
		if Config.FuelDebug then lib.print.debug("veh is nil!") end
		return false
	end
	-- return true
end